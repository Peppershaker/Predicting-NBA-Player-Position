...
class espn_player_scraper():
    def _get_team_page_urls(self, teams_overview_url):
        """Takes ESPN NBA team listing URL and returns a dict
        mapping team name to its ESPN team URL containing all 
        of the team's players"""
        
        a_style = "padding-left:0px;margin:0px;float:left;width:50px;text-decoration:none"

        teams_soup = BeautifulSoup(
            requests.get(teams_overview_url).content,
            'html.parser')
        team_a_list = teams_soup.findAll(
            'a', {'style': 'padding-top:5px;padding-left:0px;'})
    ...
    def _get_players_form_team_soup(self, team_url):
        ...
        player_table_soup = soup.findAll(
            'tr', attrs={"class": "Table2__tr Table2__tr--lg Table2__even"})

        return player_table_soup

    def _get_players_info(self, player_row_soup):
        """
        Takes soup object for the player of interest and returns 
        player info such as name, position and id in a list
        """
    ...
    def scrape_all_players(self, teams_overview_url):
        ...
        players_df = pd.DataFrame(
            player_scraping_result, columns=[
